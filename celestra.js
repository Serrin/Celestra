...
Celestra.version = "Celestra v1.20.3";
...
Celestra.extend = function extend(){var so={};if(typeof arguments[0]==="boolean"){var t=arguments[1],d=arguments[0],s=2;}else{var t=arguments[0],d=false,s=1;}for(var i=s,l=arguments.length;i<l;i++){so=arguments[i];if(so!==null&&so!==undefined){for(var a in so){if(so.hasOwnProperty(a)){if(typeof so[a]==="object"&&d){t[a]=extend(true,{},so[a]);}else{t[a]=so[a];}}}}}return t;}
Celestra.deepAssign = function deepAssign(){var s={},t=arguments[0];for(var i=1,l=arguments.length;i<l;i++){s=arguments[i];if(s!==null&&s!==undefined){for(var a in s){if(s.hasOwnProperty(a)){if(typeof s[a]==="object"){t[a]=deepAssign({},s[a]);}else{t[a]=s[a];}}}}}return t;}
...
Celestra.createFile = function createFile(fln,c,dt){var l=arguments.length;if(l>1){if(l===2){dt="text/plain";}var b=new Blob([c],{type:dt});if(window.navigator.msSaveOrOpenBlob){window.navigator.msSaveBlob(b,fln);}else{var e=window.document.createElement("a");e.href=window.URL.createObjectURL(b);e.download=fln;document.body.appendChild(e);e.click();document.body.removeChild(e);window.URL.revokeObjectURL(e.href);}}else{console.log("Celestra createFile error: too few parameters.");}}
...
Celestra.isNumeric = function isNumeric(v){return ((typeof v==="number"&&v===v)?true:(!isNaN(parseFloat(v))&&isFinite(v)));}
...
Celestra.removeCookie = function removeCookie(name,path,domain,secure,HttpOnly){var r=(document.cookie.indexOf(encodeURIComponent(name)+"=")!==-1);document.cookie=encodeURIComponent(name)+"=; expires=Thu, 01 Jan 1970 00:00:01 GMT"+"; path="+(path?path:"/")+(domain?"; domain="+domain:"")+(secure?"; secure":"")+(HttpOnly?"; HttpOnly":"")+";";return r;}
