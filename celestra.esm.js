"use strict";
/** Celestra * @version 4.5.2 esm * @see https://github.com/Serrin/Celestra/ * @license MIT */
...
function randomID(hyphens=false){var r=(new Date()).getTime().toString(16);for(var i=0;i<3;i++){r+=Math.random().toString(16).slice(2);}r=r.slice(0,32);return !hyphens?r:r.slice(0,8)+"-"+r.slice(8,12)+"-"+r.slice(12,16)+"-"+r.slice(16,20)+"-"+r.slice(20);}
...
function deepAssign(){var s={},t=arguments[0];for(var i=1,l=arguments.length;i<l;i++){s=arguments[i];if(s!==null&&s!==undefined){for(var a in s){if(Object.hasOwn(s,a)){if(typeof s[a]==="object"){t[a]=celestra.deepAssign({},s[a]);}else{t[a]=s[a];}}}}}return t;}
...
const VERSION="Celestra v4.5.2 esm";
