"use strict";
/** Celestra * @version 4.5.0 esm * @see https://github.com/Serrin/Celestra/ * @license MIT */
...
function extend(){var so={};if(typeof arguments[0]==="boolean"){var t=arguments[1],d=arguments[0],s=2;} else{var t=arguments[0],d=false,s=1;}for(var i=s,l=arguments.length;i<l;i++){so=arguments[i];if(so!==null&&so!==undefined){for(var a in so){if(Object.prototype.hasOwnProperty.call(so,a)){if(typeof so[a]==="object"&&d){t[a]=celestra.extend(true,{},so[a]);}else{t[a]=so[a];}}}}}return t;}
function deepAssign(){var s={},t=arguments[0];for(var i=1,l=arguments.length;i<l;i++){s=arguments[i];if(s!==null&&s!==undefined){for(var a in s){if(Object.prototype.hasOwnProperty.call(s,a)){if(typeof s[a]==="object"){t[a]=celestra.deepAssign({},s[a]);}else{t[a]=s[a];}}}}}return t;}
...
function strCapitalize(s){var a=[...String(s).toLowerCase()];a[0]=a[0].toUpperCase();return a.join("");}
function strUpFirst(s){var a=[...String(s)];a[0]=a[0].toUpperCase();return a.join("");}
function strDownFirst(s){var a=[...String(s)];a[0]=a[0].toLowerCase();return a.join("");}
...
function forIn(o,fn){for(var p in o){if(Object.prototype.hasOwnProperty.call(o,p)){fn(o[p],p,o);}}return o;}
const filterIn=(o,fn)=>Object.keys(o).reduce((r,p)=>{if(fn(o[p],p,o)){r[p]=o[p];}return r;},{});
function popIn(o,p){if(Object.prototype.hasOwnProperty.call(o,p)){var v=o[p];delete o[p];return v;}}
...
function domSetCSS(e,n,v){if(typeof n==="string"){e.style[n]=v;}else if(typeof n==="object"){for(var p in n){if(Object.prototype.hasOwnProperty.call(n,p)){e.style[p]=n[p];}}}}
...
function importScript(...args){for(let item of args){let scr=document.createElement("script");scr.type="text\/javascript";scr.src=item;scr.onerror=function(e){throw new URIError("Loading failed for the script with source "+e.target.src);};(document.head||document.getElementsByTagName("head")[0]).appendChild(scr);}}
function importStyle(...args){for(let item of args){let stl=document.createElement("link");stl.rel="stylesheet";stl.type="text\/css";stl.href=item;stl.onerror=function(e){throw new URIError("Loading failed for the style with source "+e.target.href);};(document.head||document.getElementsByTagName("head")[0]).appendChild(stl);}}
...
function clearCookies(path="/",domain,secure,SameSite="Lax",HttpOnly){if(typeof path==="object"){var settings=path;path=settings.path||"/";domain=settings.domain;secure=settings.secure;SameSite=settings.SameSite||"Lax";HttpOnly=settings.HttpOnly;}for(var item in celestra.getCookie()){celestra.removeCookie(item,path,domain,secure,SameSite,HttpOnly);}}
const initial=([...a])=>a.slice(0,-1);
function shuffle([...a]){for(let i=a.length-1;i>0;i--){let j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j], a[i]];}return a;}
const partition=([...a],fn)=>[a.filter(fn),a.filter((e,i,a)=>!(fn(e,i,a)))];
function groupBy(it,fn){let res={};let i=0;for(let item of it){let key=fn(item,i++);if(!(Object.prototype.hasOwnProperty.call(res,key))){res[key]=[];}res[key].push(item);}return res;}
...
function arrayAdd(a,v){if(!a.includes(v)){a.push(v);return true;}return false;}
