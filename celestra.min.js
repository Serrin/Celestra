/** Celestra * @version 2.0.5 * @see https://github.com/Serrin/Celestra/ * @license MIT */
...
function getUrlVar(n){var r={},w=window.location.search.substring(1).split("&");for(var i=0,l=w.length;i<l;i++){var e=w[i].split("=");r[decodeURIComponent(e[0])]=decodeURIComponent(e[1]);}if(n){return r[n]?r[n]:undefined;}else{return r;}}
function getUrlVarFromString(qstr,n){var r={},w=qstr.substring(1).split("&");for(var i=0,l=w.length;i<l;i++){var e=w[i].split("=");r[decodeURIComponent(e[0])]=decodeURIComponent(e[1]);}if(n){return r[n]?r[n]:undefined;}else{return r;}}
...
function fromEntries(es){var res={};if(Array.isArray(es)){es.forEach(function(e){res[e[0]]=e[1];});}else if(Object.prototype.toString.call(es)==="[object Map]"){es.forEach(function(value,key){res[key]=value;});}else if(typeof es==="object"&& typeof es.length==="number"&&es.length>=0&&es.length%1===0){for(var i=0,l=es.length;i<l;i++){res[es[i][0]]=es[i][1];}}else{throw "TypeError: iterable for celestra.fromEntries have to be array-like objects - "+JSON.stringify(es);}return res;}
...
function toObject(a){var o={length:a.length};for(var i=0,len=a.length;i<len;i++){o[""+i]=a[i];}return o;}
...
celestra.celToWindow=function(){var filter=["version","noConflict","__prevUnderscore__","celToWindow"];for(var p in celestra){if(celestra.hasOwnProperty(p)&&filter.indexOf(p)===-1){window[p]=celestra[p];}}};
celestra.qsa=qsa;
celestra.qs=qs;
...
if(typeof window!=="undefined"){window.celestra=celestra;window.Celestra=celestra;celestra.__prevUnderscore__=window._;window._=celestra;}
}(window,document));