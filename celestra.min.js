/** Celestra * @version 2.5.1 * @see https://github.com/Serrin/Celestra/ * @license MIT */
...
function qsa(s,c){if(c){var ic=(typeof c==="string")?document.querySelector(c):c;}var el=Array.prototype.slice.call((ic||document).querySelectorAll(s));el.each=el.forEach;return el;}
function qs(s,c){if(c){var ic=(typeof c==="string")?document.querySelector(c):c;}return(ic||document).querySelector(s);}
...
function getDoNotTrack(){return(navigator.doNotTrack===true||navigator.doNotTrack===1||navigator.doNotTrack==="1"||window.doNotTrack===true||window.doNotTrack===1||window.doNotTrack==="1"||navigator.msDoNotTrack===true||navigator.msDoNotTrack===1||navigator.msDoNotTrack==="1");}
...
function strRemoveTags(s){return (""+s).replace(/<[^>]*>/g," ").replace(/\s{2,}/g," ").trim();}
function strReverse(s){return (Array.from?Array.from(String(s)):String(s).split("")).reverse().join("");}
...
function merge(){if(typeof arguments[0]==="boolean"){var t=arguments[1],d=arguments[0],s=2;}else{var t=arguments[0],d=false,s=1;}for(var i=s,il=arguments.length;i<il;i++){if(Array.isArray(arguments[i])){for(var j=0,a=arguments[i],jl=a.length;j<jl;j++){if(Array.isArray(a[j])&&d){celestra.merge(true,t,a[j]);}else{t.push(a[j]);}}}else{t.push(arguments[i]);}}return t;}
function toFunction(fn){return Function.prototype.call.bind(fn);}
var bind=Function.prototype.call.bind(Function.prototype.bind);
var hasOwn=Function.prototype.call.bind(Object.prototype.hasOwnProperty);
function tap(v,fn){fn(v);return v;}
function forEach(a,fn){var t=Object.prototype.toString.call(a).replace(/^\[object (.+)\]$/,"$1").toLowerCase();if(Array.isArray(a)||t==="map"||t==="set"){a.forEach(fn);return a;}else{var a2=(Array.from?Array.from(a):Array.prototype.slice.call(a));a2.forEach(fn);if(typeof a!=="string"){return a2;}return a2.join("");}}
function map(a,fn){var t=Object.prototype.toString.call(a).replace(/^\[object (.+)\]$/,"$1").toLowerCase();if(Array.isArray(a)){return a.map(fn);}else if(t==="string"){return (Array.from?Array.from(a):Array.prototype.slice.call(a)).map(fn).join("");}else if(t==="map"){return new Map(Array.from(a).map(fn));}else if(t==="set"){return new Set(Array.from(a).map(fn));}else{return (Array.from?Array.from(a):Array.prototype.slice.call(a)).map(fn);}}
...
function domOn(el,et,fn){return el.addEventListener?el.addEventListener(et,fn):el.attachEvent("on"+et,fn);}
function domOff(el,et,fn){return el.removeEventListener?el.removeEventListener(et,fn):el.detachEvent("on"+et,fn);}
...
function isEmptyArray(v){if(Array.isArray(v)){if(v.length===0){return true;}}return false;}
function isArraylike(v){return v&&typeof v==="object"&&typeof v.length==="number"&&v.length>=0&&v.length % 1===0;}
...
function min(a){var a2=Array.from(a);if(a2.length>0){var r=a2[0];a2.forEach(function(v,i,arr){if(v<r){r=v;}});return r;}return null;}
function minIndex(a){var a2=Array.from(a);if(a2.length>0){var r=0;a2.forEach(function(v,i,arr){if(v<arr[r]){r=i;}});return r;}return null;}
function max(a){var a2=Array.from(a);if(a2.length>0){var r=a2[0];a2.forEach(function(v,i,arr){if(v>r){r=v;}});return r;}return null;}
function maxIndex(a){var a2=Array.from(a);if(a2.length>0){var r=0;a2.forEach(function(v,i,arr){if(v>arr[r]){r=i;}});return r;}return null;}
function range(start,end,step){var i=Number(start),end2=Number(end),step2=(step!==undefined?Number(step):1),res=[];while(i<=end2){res.push(i);i+=step2;}return res;}
...
var celestra={};
function uniqueArray(a){a=(Array.from?Array.from(a):Array.prototype.slice.call(a));return a.filter(function(e,i,arr){return arr.indexOf(e)===i;});}
...
celestra.noConflict=function noConflict(){window._=celestra.__prevUnderscore__;return celestra;};
celestra.__toArray__=function __toArray__(a){return (Array.isArray(a)?a:Array.from(a));};
celestra.qsa=qsa;
celestra.qs=qs;
