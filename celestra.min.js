/** Celestra * @version 2.8.0 * @see https://github.com/Serrin/Celestra/ * @license MIT */
...
function random(i,a){if(i===undefined){var i=100;}if(a===undefined){var a=i;i=0;}return Math.floor(Math.random()*(a-i+1))+i;}
function randomString(pl,sc){if(arguments.length===1){sc=false;}else if(arguments.length===0){sc=false;pl=100;}var chars="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";if(sc){chars+=",?,.:-_*$ß¤Łł÷×¸¨˝´˙`˛°˘^ˇ~§'+!%/=()[]#<>&@{}\"\\/| éáűőúöüóíÉÁŰŐÚÖÜÓÍß";}var s="",l=chars.length;for(var i=0;i<pl;i++){s+=chars[Math.floor(Math.random()*l)];}return s;}
...
function getDoNotTrack(){return (!!window.doNotTrack||!!navigator.doNotTrack||!!navigator.msDoNotTrack);}
...
function domSiblings(el){return Array.prototype.filter.call(el.parentNode.children,function(e){return (e!==el);});}
...
function isChar(v){if(typeof v==="string"){if(v.length===1){return true;}}return false;}
...
function isArraylike(v){return !!v&&(typeof v==="object"||typeof v==="string")&&typeof v.length==="number"&&v.length>=0&&v.length%1===0;}
...
function isMap(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase()==="map";}
function isSet(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase()==="set";}
function isWeakMap(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase()==="weakmap";}
function isWeakSet(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase()==="weakset";}
function isIterator(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase().indexOf("iterator")!==-1;}
function isDate(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase()==="date";}
function isRegexp(v){return Object.prototype.toString.call(v).replace(/^\[object (.+)\]$/,"$1").toLowerCase()==="regexp";}
...
function hasCookie(name){return(document.cookie.indexOf(encodeURIComponent(name)+"=")!==-1);}
...
function arrayUnion(){return Array.prototype.concat.apply([],Array.from(arguments).map(function(e){return Array.from(e);})).filter(function(e,i,arr){return arr.indexOf(e)===i;});}
function arrayIntersection(a,b){var a2=Array.from(a),b2=Array.from(b);return a2.filter(function(v){return b2.includes(v);}).filter(function(e,i,arr){return arr.indexOf(e)===i;});}
function arrayDifference(a,b){var a2=Array.from(a),b2=Array.from(b);return a2.filter(function(v){return !(b2.includes(v));}).filter(function(e,i,arr){return arr.indexOf(e)===i;});}
function arraySymmetricDifference(a,b){var a2=Array.from(a),b2=Array.from(b);return a2.filter(function(v){return !(b2.includes(v));}).concat(b2.filter(function(v){return !(a2.includes(v));})).filter(function(e,i,arr){return arr.indexOf(e)=== i;});}
function setUnion(){return new Set(Array.prototype.concat.apply([],Array.from(arguments).map(function(e){return Array.from(e);})));}
function setIntersection(a,b){return new Set(Array.from(a).filter(function(v){return b.has(v);}));}
function setDifference(a,b){return new Set(Array.from(a).filter(function(v){return !(b.has(v));}));}
function setSymmetricDifference(a,b){return new Set(Array.from(a).filter(function(v){return !(b.has(v));}).concat(Array.from(b).filter(function(v){return !(a.has(v));})));}
function arrayKeys(a){return Array.from(a).map(function(v,i){return i;});}
function arrayValues(a){return Array.from(a);}
function arrayEntries(a){return Array.from(a).map(function(v,i){return [i,v];});}
function isSuperset(sup,sub){var sup2=celestra.__toArray__(sup);return celestra.__toArray__(sub).every(function(v){return sup2.includes(v);});}
function min(a){var a2=celestra.__toArray__(a);if(a2.length>0){var r=a2[0];a2.forEach(function(v,i,arr){if(v<r){r=v;}});return r;}return null;}
function minIndex(a){var a2=celestra.__toArray__(a);if(a2.length>0){var r=0;a2.forEach(function(v,i,arr){if(v<arr[r]){r=i;}});return r;}return null;}
function max(a){var a2=celestra.__toArray__(a);if(a2.length>0){var r=a2[0];a2.forEach(function(v,i,arr){if(v>r){r=v;}});return r;}return null;}
function maxIndex(a){var a2=celestra.__toArray__(a);if(a2.length>0){var r=0;a2.forEach(function(v,i,arr){if(v>arr[r]){r=i;}});return r;}return null;}
function arrayRepeat(v,n){return Array(arguments.length===2?n:100).fill(v);}
function arrayCycle(a,n){var a2=Array.from(a);return Array(arguments.length===2?n:100).fill(a2).flat();}
function arrayRange(start,end,step){var i=Number(start),end2=Number(end),step2=(step!==undefined?Number(step):1),res=[];while(i<=end2){res.push(i);i+=step2;}return res;}
var range = arrayRange;
function toPairs(a,b){var a2=Array.from(a),b2=Array.from(b);var l=(a2.length<b2.length?a2.length:b2.length);var res=[];for(var i=0;i<l;i++){res.push([a2[i],b2[i]]);}return res;}
...
function unzip(a){var a2=Array.from(a).map(function(v){return Array.from(v);});var res=[],i,j,l1=a2[0].length,l2=a2.length;for(i=0;i<l1;i++){res.push([]);}for(i=0;i<l1;i++){for(j=0;j<l2;j++){res[i].push(a2[j][i]);}}return res;}
function uniqueArray(a){return celestra.__toArray__(a).filter(function(e,i,arr){return arr.indexOf(e)===i;});}
...
function item(a,i){var a2=celestra.__toArray__(a);return a2[(i<0?a2.length+i:i)];}
...
var celestra={version:"Celestra v2.8.0", noConflict:noConflict, __toArray__:__toArray__, qsa:qsa, qs:qs, domReady:domReady, random:random, randomString:randomString, b64Encode:b64Encode, b64Decode:b64Decode, javaHash:javaHash, inherit:inherit, importScript:importScript, importScripts:importScripts, importStyle:importStyle, importStyles:importStyles, getUrlVar:getUrlVar, getUrlVarFromString:getUrlVarFromString, obj2string:obj2string, getType:getType, extend:extend, deepAssign:deepAssign, getFullscreen:getFullscreen, setFullscreenOn:setFullscreenOn, setFullscreenOff:setFullscreenOff, getLocation:getLocation, getDoNotTrack:getDoNotTrack, form2array:form2array, form2string:form2string, strRemoveTags:strRemoveTags, strReverse:strReverse, createFile:createFile, forIn:forIn, mapIn:mapIn, toFunction:toFunction, bind:bind, hasOwn:hasOwn, constant:constant, identity:identity, noop:noop, T:T, F:F, domCreate:domCreate, domToElement:domToElement, domGetCSS:domGetCSS, domSetCSS:domSetCSS, domFadeIn:domFadeIn, domFadeOut:domFadeOut, domFadeToggle:domFadeToggle, domHide:domHide, domShow:domShow, domToggle:domToggle, domIsHidden:domIsHidden, domOn:domOn, domOff:domOff, domTrigger:domTrigger, domSiblings:domSiblings, getJson:getJson, getText:getText, getAjax:getAjax, postAjax:postAjax, getCors:getCors, postCors:postCors, isEqual:isEqual, isString:isString, isChar:isChar, isNumber:isNumber, isInteger:isInteger, isFloat:isFloat, isNumeric:isNumeric, isBoolean:isBoolean, isObject:isObject, isEmptyObject:isEmptyObject, isFunction:isFunction, isArray:isArray, isEmptyArray:isEmptyArray, isArraylike:isArraylike, isNull:isNull, isUndefined:isUndefined, isNullOrUndefined:isNullOrUndefined, isNil:isNil, isPrimitive:isPrimitive, isSymbol:isSymbol, isMap:isMap, isSet:isSet, isWeakMap:isWeakMap, isWeakSet:isWeakSet, isIterator:isIterator, isDate:isDate, isRegexp:isRegexp, isElement:isElement, isIterable:isIterable, isBigInt:isBigInt, setCookie:setCookie, getCookie:getCookie, hasCookie:hasCookie, removeCookie:removeCookie, clearCookies:clearCookies, forEach:forEach, map:map, arrayUnion:arrayUnion, arrayIntersection:arrayIntersection, arrayDifference:arrayDifference, arraySymmetricDifference:arraySymmetricDifference, setUnion:setUnion, setIntersection:setIntersection, setDifference:setDifference, setSymmetricDifference:setSymmetricDifference, arrayKeys:arrayKeys, arrayValues:arrayValues, arrayEntries:arrayEntries, isSuperset:isSuperset, min:min, minIndex:minIndex, max:max, maxIndex:maxIndex, arrayRepeat:arrayRepeat, arrayCycle:arrayCycle, range:range, arrayRange:arrayRange, toPairs:toPairs, zip:zip, unzip:unzip, uniqueArray:uniqueArray, uniquePush:uniquePush, arrayClear:arrayClear, arrayRemove:arrayRemove, item:item, arrayMerge:arrayMerge};
