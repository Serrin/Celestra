/** Celestra * @version 2.2.0 * @see https://github.com/Serrin/Celestra/ * @license MIT */
...
if(!Object.entries){Object.entries=function(obj){var res=[];Object.keys(obj).forEach(function(e){res.push([e,obj[e]]);});return res;};}
if(!Object.values){Object.values=function(obj){var res=[];Object.keys(obj).forEach(function(e){res.push(obj[e]);});return res;};}
...
Math.hypot=function(x,y){var max=0;var s=0;for(var i=0;i<arguments.length;i+=1){var arg=Math.abs(Number(arguments[i]));if(arg>max){s*=(max/arg)*(max/arg);max=arg;}s+=arg===0&&max===0?0:(arg/max)*(arg/max);}return max===1/0?1/0:max*Math.sqrt(s);};
...
function removeTags(s){return (""+s).replace(/<[^>]*>/g," ").replace(/\s{2,}/g," ").trim();}
...
function forEach(a,fn){var t=Object.prototype.toString.call(a).replace(/^\[object (.+)\]$/,"$1").toLowerCase();if(Array.isArray(a)||t==="map"||t==="set"){a.forEach(fn);return a;}else{var a2=(Array.from?Array.from(a):Array.prototype.slice.call(a));a2.forEach(fn);if(typeof a!=="string"){return a2;}return a;}}
...
function forIn(o,fn){for(var p in o){if(o.hasOwnProperty(p)){fn(o[p],p,o);}}}
...
function domToElement(s){const e=document.createElement('div');e.innerHTML=s;return e.firstElementChild;}
...
celestra.noConflict=function(){window._=celestra.__prevUnderscore__;return celestra;};
celestra.qsa=qsa;
celestra.qs=qs;
