/** Celestra FP * @version 1.20.2 * @see https://github.com/Serrin/Celestra/ * @license MIT */
"use strict";
/* polyfills */
...
if(!isNaN){var isNaN=function isNaN(v){return Number.isNaN(Number(v));};}
...
function random(i,a){if(a===undefined){var a=i;i=0;}return Math.floor(Math.random()*(a-i+1))+i;}
...
function getScripts(s){if(Array.isArray(s)){s.forEach(function(e){if(e.success){getScript(e.url,e.success);}else{getScript(e.url);}});}}
...
function getStyles(s){if(Array.isArray(s)){s.forEach(function(e){if(e.success){getStyle(e.href,e.success);}else{getStyle(e.href);}});}}
...
function deepAssign(){var s={},t=arguments[0];for(var i=1,l=arguments.length;i<l;i++){s=arguments[i];if(s!==null&&s!==undefined){for(var a in s){if(s.hasOwnProperty(a)){if(typeof s[a]==="object"){t[a]=deepAssign({},s[a]);}else{t[a]=s[a];}}}}}return t;}
...
function getDoNotTrack(){if(navigator.doNotTrack===true||navigator.doNotTrack===1||navigator.doNotTrack==="1"||window.doNotTrack===true||window.doNotTrack===1||window.doNotTrack==="1"||navigator.msDoNotTrack===true||navigator.msDoNotTrack===1||navigator.msDoNotTrack==="1"){return true;}return false;}
...
function domFadeIn(e,dur,disp){var s=e.style,step=25/(dur||500);s.opacity=(s.opacity||0);s.display=(disp||"");(function fade(){(s.opacity=parseFloat(s.opacity)+step)>1?s.opacity=1:setTimeout(fade,25);})();}
...
function domFadeToggle(e,dur,disp){if((window.getComputedStyle?getComputedStyle(e,null):e.currentStyle).display==="none"){domFadeIn(e,dur,(disp||""));}else{domFadeOut(e,dur);}}
